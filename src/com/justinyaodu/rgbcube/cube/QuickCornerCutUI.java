...
public class QuickCornerCutUI extends ControlBox<CubeMasker>
{
	public QuickCornerCutUI(CubeMasker cubeMasker)
	{
	...
	{
		GridPane gridPane = FXHelper.gridPaneWithRowHeaders("Corner", CornerCutUI.DIAGONAL_LABEL, CornerCutUI.CUBE_LABEL);
		...
		cornerComboBox.getSelectionModel().selectedItemProperty().addListener((o, oldValue, newValue) ->
		{
			...
			cubeMasker.diagonalCutDepthProperty(newValue).bindBidirectional(diagonalSlider.valueProperty());
			cubeMasker.cubeCutDepthProperty(newValue).bindBidirectional(cubeSlider.valueProperty());
		});

		// initialize value
		cornerComboBox.getSelectionModel().select(RgbCube.Corner.RED);

		gridPane.addColumn(1, cornerComboBox, diagonalSlider, cubeSlider);
